...
public class ClientHandler implements Runnable{
    ...
    public ClientHandler(Socket socket, Connection con) {
        try {
            createMatchStmt = con.prepareStatement("INSERT INTO match (player1) VALUES(?)");
        ...
        while (goFlag && socket.isConnected()) {
            try {
                ...
                if(command.equals("goodbye")) {
                    ...
                    tellClient(command+","+name+": "+value[0]);
                    opponent.tellClient(command+","+name+": "+value[0]);
